// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'user_info.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

UserInfo _$UserInfoFromJson(Map<String, dynamic> json) {
  return _UserInfo.fromJson(json);
}

/// @nodoc
mixin _$UserInfo {
  String get sub => throw _privateConstructorUsedError;
  String get email => throw _privateConstructorUsedError;
  dynamic get role => throw _privateConstructorUsedError;
  @JsonKey(name: 'preferred_username')
  String get preferredUsername => throw _privateConstructorUsedError;
  String get name => throw _privateConstructorUsedError;
  @JsonKey(name: 'email_verified')
  bool get emailVerified => throw _privateConstructorUsedError;
  String get eid => throw _privateConstructorUsedError;
  String get deid => throw _privateConstructorUsedError;
  String get eki => throw _privateConstructorUsedError;
  @JsonKey(name: 'user_abbr')
  String get userAbbr => throw _privateConstructorUsedError;

  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $UserInfoCopyWith<UserInfo> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $UserInfoCopyWith<$Res> {
  factory $UserInfoCopyWith(UserInfo value, $Res Function(UserInfo) then) =
      _$UserInfoCopyWithImpl<$Res, UserInfo>;
  @useResult
  $Res call(
      {String sub,
      String email,
      dynamic role,
      @JsonKey(name: 'preferred_username') String preferredUsername,
      String name,
      @JsonKey(name: 'email_verified') bool emailVerified,
      String eid,
      String deid,
      String eki,
      @JsonKey(name: 'user_abbr') String userAbbr});
}

/// @nodoc
class _$UserInfoCopyWithImpl<$Res, $Val extends UserInfo>
    implements $UserInfoCopyWith<$Res> {
  _$UserInfoCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? sub = null,
    Object? email = null,
    Object? role = freezed,
    Object? preferredUsername = null,
    Object? name = null,
    Object? emailVerified = null,
    Object? eid = null,
    Object? deid = null,
    Object? eki = null,
    Object? userAbbr = null,
  }) {
    return _then(_value.copyWith(
      sub: null == sub
          ? _value.sub
          : sub // ignore: cast_nullable_to_non_nullable
              as String,
      email: null == email
          ? _value.email
          : email // ignore: cast_nullable_to_non_nullable
              as String,
      role: freezed == role
          ? _value.role
          : role // ignore: cast_nullable_to_non_nullable
              as dynamic,
      preferredUsername: null == preferredUsername
          ? _value.preferredUsername
          : preferredUsername // ignore: cast_nullable_to_non_nullable
              as String,
      name: null == name
          ? _value.name
          : name // ignore: cast_nullable_to_non_nullable
              as String,
      emailVerified: null == emailVerified
          ? _value.emailVerified
          : emailVerified // ignore: cast_nullable_to_non_nullable
              as bool,
      eid: null == eid
          ? _value.eid
          : eid // ignore: cast_nullable_to_non_nullable
              as String,
      deid: null == deid
          ? _value.deid
          : deid // ignore: cast_nullable_to_non_nullable
              as String,
      eki: null == eki
          ? _value.eki
          : eki // ignore: cast_nullable_to_non_nullable
              as String,
      userAbbr: null == userAbbr
          ? _value.userAbbr
          : userAbbr // ignore: cast_nullable_to_non_nullable
              as String,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$_UserInfoCopyWith<$Res> implements $UserInfoCopyWith<$Res> {
  factory _$$_UserInfoCopyWith(
          _$_UserInfo value, $Res Function(_$_UserInfo) then) =
      __$$_UserInfoCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {String sub,
      String email,
      dynamic role,
      @JsonKey(name: 'preferred_username') String preferredUsername,
      String name,
      @JsonKey(name: 'email_verified') bool emailVerified,
      String eid,
      String deid,
      String eki,
      @JsonKey(name: 'user_abbr') String userAbbr});
}

/// @nodoc
class __$$_UserInfoCopyWithImpl<$Res>
    extends _$UserInfoCopyWithImpl<$Res, _$_UserInfo>
    implements _$$_UserInfoCopyWith<$Res> {
  __$$_UserInfoCopyWithImpl(
      _$_UserInfo _value, $Res Function(_$_UserInfo) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? sub = null,
    Object? email = null,
    Object? role = freezed,
    Object? preferredUsername = null,
    Object? name = null,
    Object? emailVerified = null,
    Object? eid = null,
    Object? deid = null,
    Object? eki = null,
    Object? userAbbr = null,
  }) {
    return _then(_$_UserInfo(
      sub: null == sub
          ? _value.sub
          : sub // ignore: cast_nullable_to_non_nullable
              as String,
      email: null == email
          ? _value.email
          : email // ignore: cast_nullable_to_non_nullable
              as String,
      role: freezed == role
          ? _value.role
          : role // ignore: cast_nullable_to_non_nullable
              as dynamic,
      preferredUsername: null == preferredUsername
          ? _value.preferredUsername
          : preferredUsername // ignore: cast_nullable_to_non_nullable
              as String,
      name: null == name
          ? _value.name
          : name // ignore: cast_nullable_to_non_nullable
              as String,
      emailVerified: null == emailVerified
          ? _value.emailVerified
          : emailVerified // ignore: cast_nullable_to_non_nullable
              as bool,
      eid: null == eid
          ? _value.eid
          : eid // ignore: cast_nullable_to_non_nullable
              as String,
      deid: null == deid
          ? _value.deid
          : deid // ignore: cast_nullable_to_non_nullable
              as String,
      eki: null == eki
          ? _value.eki
          : eki // ignore: cast_nullable_to_non_nullable
              as String,
      userAbbr: null == userAbbr
          ? _value.userAbbr
          : userAbbr // ignore: cast_nullable_to_non_nullable
              as String,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$_UserInfo with DiagnosticableTreeMixin implements _UserInfo {
  const _$_UserInfo(
      {required this.sub,
      required this.email,
      this.role,
      @JsonKey(name: 'preferred_username') required this.preferredUsername,
      required this.name,
      @JsonKey(name: 'email_verified') required this.emailVerified,
      required this.eid,
      required this.deid,
      required this.eki,
      @JsonKey(name: 'user_abbr') required this.userAbbr});

  factory _$_UserInfo.fromJson(Map<String, dynamic> json) =>
      _$$_UserInfoFromJson(json);

  @override
  final String sub;
  @override
  final String email;
  @override
  final dynamic role;
  @override
  @JsonKey(name: 'preferred_username')
  final String preferredUsername;
  @override
  final String name;
  @override
  @JsonKey(name: 'email_verified')
  final bool emailVerified;
  @override
  final String eid;
  @override
  final String deid;
  @override
  final String eki;
  @override
  @JsonKey(name: 'user_abbr')
  final String userAbbr;

  @override
  String toString({DiagnosticLevel minLevel = DiagnosticLevel.info}) {
    return 'UserInfo(sub: $sub, email: $email, role: $role, preferredUsername: $preferredUsername, name: $name, emailVerified: $emailVerified, eid: $eid, deid: $deid, eki: $eki, userAbbr: $userAbbr)';
  }

  @override
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties
      ..add(DiagnosticsProperty('type', 'UserInfo'))
      ..add(DiagnosticsProperty('sub', sub))
      ..add(DiagnosticsProperty('email', email))
      ..add(DiagnosticsProperty('role', role))
      ..add(DiagnosticsProperty('preferredUsername', preferredUsername))
      ..add(DiagnosticsProperty('name', name))
      ..add(DiagnosticsProperty('emailVerified', emailVerified))
      ..add(DiagnosticsProperty('eid', eid))
      ..add(DiagnosticsProperty('deid', deid))
      ..add(DiagnosticsProperty('eki', eki))
      ..add(DiagnosticsProperty('userAbbr', userAbbr));
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$_UserInfo &&
            (identical(other.sub, sub) || other.sub == sub) &&
            (identical(other.email, email) || other.email == email) &&
            const DeepCollectionEquality().equals(other.role, role) &&
            (identical(other.preferredUsername, preferredUsername) ||
                other.preferredUsername == preferredUsername) &&
            (identical(other.name, name) || other.name == name) &&
            (identical(other.emailVerified, emailVerified) ||
                other.emailVerified == emailVerified) &&
            (identical(other.eid, eid) || other.eid == eid) &&
            (identical(other.deid, deid) || other.deid == deid) &&
            (identical(other.eki, eki) || other.eki == eki) &&
            (identical(other.userAbbr, userAbbr) ||
                other.userAbbr == userAbbr));
  }

  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hash(
      runtimeType,
      sub,
      email,
      const DeepCollectionEquality().hash(role),
      preferredUsername,
      name,
      emailVerified,
      eid,
      deid,
      eki,
      userAbbr);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$_UserInfoCopyWith<_$_UserInfo> get copyWith =>
      __$$_UserInfoCopyWithImpl<_$_UserInfo>(this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$_UserInfoToJson(
      this,
    );
  }
}

abstract class _UserInfo implements UserInfo {
  const factory _UserInfo(
      {required final String sub,
      required final String email,
      final dynamic role,
      @JsonKey(name: 'preferred_username')
          required final String preferredUsername,
      required final String name,
      @JsonKey(name: 'email_verified')
          required final bool emailVerified,
      required final String eid,
      required final String deid,
      required final String eki,
      @JsonKey(name: 'user_abbr')
          required final String userAbbr}) = _$_UserInfo;

  factory _UserInfo.fromJson(Map<String, dynamic> json) = _$_UserInfo.fromJson;

  @override
  String get sub;
  @override
  String get email;
  @override
  dynamic get role;
  @override
  @JsonKey(name: 'preferred_username')
  String get preferredUsername;
  @override
  String get name;
  @override
  @JsonKey(name: 'email_verified')
  bool get emailVerified;
  @override
  String get eid;
  @override
  String get deid;
  @override
  String get eki;
  @override
  @JsonKey(name: 'user_abbr')
  String get userAbbr;
  @override
  @JsonKey(ignore: true)
  _$$_UserInfoCopyWith<_$_UserInfo> get copyWith =>
      throw _privateConstructorUsedError;
}
